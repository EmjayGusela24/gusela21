...
const AdminVotersList: React.FC<{
  ...
  const fetchStudentsWithVotes = async () => {
    ...
    const { data: studentsData, error: studentsError } = await supabase
      .from("students")
      .select("id, name, grade, section, age, has_voted, voted_at, vote_location")
      .order("name");
    ...
    <div className="screen-content content-max-width">
      ...
      <div className="admin-landscape-layout">
        <div className="admin-sidebar">
          {/* Filter Card */}
          <div className="card-box" style={{ padding: "20px", background: "white" }}>
            <h3 style={{ marginBottom: "16px", fontSize: "15px", color: "var(--primary-navy)", display: "flex", alignItems: "center", gap: "8px" }}>
              <span className="material-symbols-outlined" style={{ fontSize: "20px" }}>filter_alt</span>
              Filter Registry
            </h3>
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              <label style={{ fontWeight: 600, fontSize: "12px", color: "var(--text-light)" }}>
                Filter by Grade:
              </label>
              <select
                value={gradeFilter}
                onChange={(e) => setGradeFilter(e.target.value)}
                style={{ width: "100%", padding: "10px 14px", border: "1px solid var(--border-light)", borderRadius: "6px", background: "var(--bg-main)", fontSize: "14px", boxSizing: "border-box" }}
              >
                ...
                {filteredStudents.map((s) => (
                  <div key={s.id} style={{ padding: "20px 24px", background: "linear-gradient(145deg, #ffffff, #f8fafc)", borderRadius: "16px", border: "1px solid var(--border-light)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "24px", transition: "transform 0.2s ease, box-shadow 0.2s ease", flexWrap: "wrap" }} className="hover-lift">
                    <div style={{ display: "flex", alignItems: "center", gap: "20px", minWidth: "220px" }}>
                      <StudentPhoto studentId={s.id} name={s.name} />
                      <div>
